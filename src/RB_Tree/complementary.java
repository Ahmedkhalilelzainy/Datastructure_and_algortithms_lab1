...
public class complementary<T extends Comparable<T>> {
    ...
    private boolean balanceVal;
    public boolean callBalance(RBNode<T> root){
        balanceVal = true;
        isBalanced1(root);
        return balanceVal;
    }

    public int isBalanced1(RBNode<T> currentNode){
        if( currentNode.getLeftChild() == null && currentNode.getRightChild() == null ){
            return 0;
        }
        int leftBalance = isBalanced1(currentNode.getLeftChild());
        int rightBalance = isBalanced1(currentNode.getRightChild());
        if( leftBalance != rightBalance ){
            balanceVal = false;
            return -1;
        } else {
            return ( currentNode.getColor() == Color.BLACK )? leftBalance+1 : leftBalance ;
        }
    }
}
